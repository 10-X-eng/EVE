...
from eve.updates import RELEASES, UpdateChecker, check_release, select_release
...
def release(version="0.3.0", platforms=("windows-x64", "macos-arm64")):
    ...
    return {"tag_name": tag, "draft": False, "prerelease": True, "published_at": "2026-09-20T00:00:00Z",
            "assets": [{"name": name, "state": "uploaded", "size": 100,
                       ...
                       (f"EVE-{version}-{platform}.zip", f"EVE-{version}-{platform}.zip.sha256")]}
...
class ReleaseTests(unittest.TestCase):
    ...
    def test_http_request_is_bounded_and_has_no_authentication(self):
        ...
        with patch("eve.updates.urlopen", return_value=response) as urlopen:
            self.assertEqual(check_release()["version"], "0.3.0")
        request = urlopen.call_args.args[0]
        self.assertTrue(request.full_url.startswith("https://api.github.com/repos/10-X-eng/EVE/releases?"))
        ...
        with patch("eve.updates.urlopen", return_value=response), self.assertRaises(ValueError):
            check_release()

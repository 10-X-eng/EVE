"""Offline CLI qualification adapted from Nous Research (MIT).
See addin/STEVE/steve/claude_native/LICENSE and NOTICE.md.
Never uses the real account: isolated config and a synthetic loopback peer.
"""
...
from steve.claude_native import directsdk
from steve.claude_setup import resolve_claude
from steve.loopback_http import ThreadingLoopbackHTTPServer as ThreadingHTTPServer
import unittest
...
def events(mode):
    ...
    if mode in ('tools', 'tool_max'):
        blocks += [{'type':'tool_use', 'id':'host_tool_1', 'name':'mcp__steve__read_file', 'input':{'path':'fixture.txt'}}]
...
class Peer(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.server.mode == 'cancel':
            try:
                self.rfile.read(1)  # Windows can report reset/abort instead of EOF.
...
@unittest.skipUnless(CLAUDE, "Install Claude Code for the offline native protocol qualification")
class ClaudeNativeRuntimeTests(unittest.TestCase):
    def test_offline_native_protocol_and_cancellation(self):
        result = run(CLAUDE[0])

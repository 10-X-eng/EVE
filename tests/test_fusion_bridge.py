...
def load_bridge():
    ...
    with patch.dict(sys.modules, modules):
        spec = importlib.util.spec_from_file_location("steve.test_fusion_tools", ROOT / "addin/STEVE/steve/fusion_tools.py")
...
""")
    ...
    def test_application_created_document_transfers_pin(self):
        ...
        replacement = Obj(name="Created by STEVE", products=Collection(self.host.activeProduct))
    ...
    def test_job_restores_original_pin_after_working_in_another_chat(self):
        ...
        original.isValid = True
        snapshot = self.bridge.message_context("send")
        self.host.activeDocument = Obj(name="Other document", products=Collection())
        self.bridge.message_context("send")
        resumed = self.bridge.message_context("resume:" + snapshot["task_key"])
        self.assertEqual(resumed["document_id"], snapshot["document_id"])
        self.assertIs(self.bridge.context()["document"], original)
        self.assertIsNot(self.host.activeDocument, original)  # Never activate tabs behind the user's back.
        original.isValid = False
        with self.assertRaisesRegex(Exception, "original document"):
            self.bridge.message_context("resume:" + snapshot["task_key"])

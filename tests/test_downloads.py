...
from eve.downloads import UpdateDownloader, download_package, downloads_folder
from eve.updates import RELEASES
...
class DownloadTests(unittest.TestCase):
    def setUp(self):
        ...
        self.target = patch("eve.downloads.host_target", return_value="x86_64-pc-windows-msvc")
        self.target.start()
        self.addCleanup(self.target.stop)
        self.filename = "EVE-0.3.0-windows-x64.zip"
    ...
    def test_macos_downloads_path_uses_current_users_home(self):
        with patch("eve.downloads.sys.platform", "darwin"), patch("eve.downloads.Path.home", return_value=self.folder):
    ...
    def test_background_download_coalesces_and_does_not_publish_after_close(self):
        ...
        try:
            with patch("eve.downloads.download_package", side_effect=fetch) as download:

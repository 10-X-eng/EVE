...
const source = fs.readFileSync(path.join(__dirname, '../addin/EVE/panel/panel.js'), 'utf8');
...
(async () => {
  ...
  context.renderControls();
  assert.equal(elements.get('installed-version').textContent, 'EVE 0.2.0');
  assert.equal(elements.get('update-title').textContent, 'EVE 0.3.0 is available');

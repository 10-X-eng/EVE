...
from steve.transport import RuntimeUnavailable, VERSION, data_home, host_target, runtime_command
...
class RuntimeSetupTests(unittest.TestCase):
    ...
    def test_missing_incomplete_and_valid_installations(self):
        for target, suffix in ((WINDOWS_TARGET, ".exe"), (MAC_TARGET, "")):
            with self.subTest(target=target):
                ...
                (root / "bin").mkdir()
                executable.touch()
                (root / "steve-runtime.json").write_text(json.dumps({"version": VERSION, "target": target}))
    ...
    def test_runtime_built_for_another_platform_is_rejected(self):
        ...
        (root / "steve-runtime.json").write_text(json.dumps({"version": VERSION, "target": "x86_64-apple-darwin"}))
        ...
        (root / "steve-runtime.json").write_text(json.dumps({"version": VERSION}))
    ...
    def test_data_home_per_platform(self):
        ...
        with patch.dict(os.environ, {"LOCALAPPDATA": str(local)}):
            self.assertEqual(data_home("win32"), local / "STEVE")
        self.assertEqual(data_home("darwin"), Path.home() / "Library" / "Application Support" / "STEVE")
        with patch.dict(os.environ, {"XDG_DATA_HOME": str(local)}):
            self.assertEqual(data_home("linux"), local / "STEVE")

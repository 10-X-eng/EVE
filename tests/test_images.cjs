...
const root = path.join(__dirname, '../addin/STEVE/panel');
...
const images = vm.runInContext('SteveImages',context);
...
(async()=>{
  ...
  // A late image reload from the old provider cannot populate the new provider's draft.
  elements.get('message').value='';
  let reloaded;
  context.pendingAssets=()=>new Promise(resolve=>{reloaded=resolve;});
  run(`draftImages=[];state.provider='chatgpt';state.threadId='old-chat';bridge=pendingAssets;`);
  const reusing=context.reuseMessage({text:'Old provider draft',images:[{id:'fixture',name:'Reference'}]});
  run(`state.provider='grok';state.threadId=null;`);
  reloaded({images:{fixture:png}});await reusing;
  assert.equal(run('draftImages.length'),0);
  assert.equal(elements.get('message').value,'');

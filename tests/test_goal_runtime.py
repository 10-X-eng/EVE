"""Bundled-runtime goals with a local inference fixture; no paid calls or Fusion edits."""
import json
    ...
    def test_budget_continuation_pause_resume_completion_and_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            def idle(controller, status):
                eventually(lambda: controller.state["goal"] and controller.state["goal"]["status"] == status
                           and not controller.state["busy"] and not controller.state["goalBusy"], 15)
            ...
            with patch("steve.grok_transport.GrokGateway", side_effect=lambda auth: GrokGateway(auth, opener=upstream)):
                controller = connect()
                try:
                    controller.dispatch("goal", {"command": "set", "objective": "Inspect the fixture", "tokenBudget": 100},
                    ...
                    phase[0] = "pause"
                    controller.dispatch("goal", {"command": "resume", "tokenBudget": None},
                        capture_context=lambda _: {"document_id": "fixture", "name": "Fixture bracket", "task_key": "fixture-key"})
                    eventually(lambda: fusion.pending is not None and not controller.state["goalBusy"], 10)
                    controller.dispatch("stop")
                    idle(controller, "paused")
                    self.assertIsNone(controller.state["goal"]["tokenBudget"])
                    complete, cancelled = fusion.pending
                    self.assertTrue(cancelled())
                    complete({"ok": False, "error": "Stopped fixture"})
                    phase[0] = "complete"
                    controller.dispatch("goal", {"command": "resume"},
                        capture_context=lambda _: {"document_id": "fixture", "name": "Fixture bracket", "task_key": "fixture-key"})
                ...
                try:
                    ...
                    self.assertEqual(controller.state["goal"]["status"], "complete")
                    self.assertEqual(controller.state["goal"]["objective"], "Inspect the fixture")
                    controller.dispatch("goal", {"command": "set", "objective": "Verify revised fixture", "tokenBudget": 75},
                        capture_context=lambda _: {"document_id": "fixture", "name": "Fixture bracket", "task_key": "fixture-key"})
                    idle(controller, "budgetLimited")
                    self.assertEqual(controller.state["goal"]["tokensUsed"], 75,
                    ...
                    count = len(requests)
                    controller.dispatch("goal", {"command": "clear"})
                    eventually(lambda: controller.state["goal"] is None and not controller.state["goalBusy"])
                    self.assertEqual(len(requests), count)
                    # Leave an active goal on disk, as after an app shutdown.
                    phase[0] = "pause"
                    fusion.pending = None
                    controller.dispatch("goal", {"command": "set", "objective": "Check saved active goal"},
                        capture_context=lambda _: {"document_id": "fixture", "name": "Fixture bracket", "task_key": "fixture-key"})
                    eventually(lambda: fusion.pending is not None and not controller.state["goalBusy"], 10)
                finally:
                    controller.close()
                ...
                try:
                    ...
                    controller.dispatch("openHistory", {"threadId": thread})
                    eventually(lambda: controller.thread_id == thread and not controller.state["busy"], 10)
                    self.assertEqual(controller.state["goal"]["status"], "paused")
                    self.assertEqual(len(requests), count)
                    self.assertFalse(controller.state["goalHasTarget"])
                finally:
                    controller.close()
                    controller._worker.join(5)

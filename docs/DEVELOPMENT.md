...
`STEVE.manifest` owns the STEVE version; `steve.version` supplies it to the panel, Codex handshake, and packaging scripts. Bump that version and update the release notes and installation examples in a PR. Merging to main publishes the new version after Windows and macOS package verification succeeds.

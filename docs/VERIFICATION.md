...
- Unreleased 0.8.1, issue #65: paused-job recovery passes 471 local Python tests (11 platform/installer skips) and all six panel/browser suites. Regression cases cover missing turn completion, either paused/idle event order, activity refresh racing a new turn, unrelated threads, preserved document binding and active Fusion tools. The running installed add-in was inspected read-only; the reported state had already resumed, so the original screenshot's exact event sequence could not be recovered. No release or live add-in replacement was performed.

...
GUIDES = {
    ...
    'sheet_metal': {
        ...
        'supplier': 'When rmfgConnection is connected, handle catalog lookup, scoped upload, report retrieval and rechecks as part of the design task. Match catalog material and thickness to established requirements; ask only when missing, ambiguous or unavailable. Do not substitute a different alloy or thickness silently. Apply fixes within the design intent, then recheck changed geometry. When asked for pricing or checkout, use rmfg_checkout with saved part jobs and requested quantities; create the ready cart so the customer can review and pay on RMFG. Present actionable findings and unresolved limits, not upload/job-management instructions.',
        'unchecked': 'Flat pattern creation is a modification. Bend sequence, springback, tooling collision and imported-solid unfolding require additional validation. Optional RMFG supplier analysis uses rmfg_materials and fusion_rmfg; once connected, relevant part snapshots upload automatically during DFM checks. Native checks need no supplier connection.',
    },
    'fdm': {

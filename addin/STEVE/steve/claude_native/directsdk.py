...
from ..claude_setup import INSTALL_HINT, resolve_claude
...
def normalize_input_schema(schema):
    """Normalize top-level schema combinations; STEVE validates tool arguments."""
    # Fusion's tool handlers validate arguments; keep nested JSON Schema intact.
...
def kill_process_tree(process):
    ...
    if process.poll() is not None:
        return
    if os.name == 'nt':
        subprocess.run(['taskkill', '/F', '/T', '/PID', str(process.pid)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False, creationflags=subprocess.CREATE_NO_WINDOW)
        return
    try:
        os.killpg(process.pid, signal.SIGKILL)  # windows-footgun: ok â€” the nt branch above never reaches this line
...
class Client:
    def __init__(self, command=None, args=None, env=None, timeout=180, **_):
        ...
        command = command or source_env.get('STEVE_CLAUDE_COMMAND') or 'claude'
    ...
    def cancel(self):
        ...
        with self._lock:
            # STEVE creates one client per request. Latch cancellation even before spawn.
            self._closed = True
    ...
    def _run(self, request, kwargs, body, manifest, names, system, frames):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix='claude-directsdk-', ignore_cleanup_errors=True) as tmp:
                ...
                mcp = {'mcpServers': {'steve': {'type': 'http', 'url': inventory.url}}}
                ...
                env.update(DISABLE_AUTOUPDATER='1', DISABLE_TELEMETRY='1', DISABLE_ERROR_REPORTING='1', ENABLE_TOOL_SEARCH='false', CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC='1', CLAUDE_CODE_MAX_RETRIES='0', DISABLE_AUTO_COMPACT='1', DISABLE_COMPACT='1')

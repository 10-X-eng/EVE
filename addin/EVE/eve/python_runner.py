    ...
    try:
        ...
        for node in ast.walk(tree):
            if (context.get("targetPinned") and isinstance(node, ast.Attribute)
                ...
                raise ToolError("live_context_access", f"Line {node.lineno}: {node.attr} follows the user's current UI and can retarget the task. Use EVE's pinned context instead.")
        ...
        if not any(isinstance(node, ast.FunctionDef) and node.name == "run" for node in tree.body):
            raise ValueError("Define def run(context) with the operation inside it. EVE calls run once.")
        if cancelled():
            raise ScriptStopped("Operation cancelled before execution.")
        namespace = {"__name__": "__eve_script__", "__builtins__": {**vars(builtins), "print": capture}}
        sys.settrace(trace)

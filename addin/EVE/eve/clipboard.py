...
try {
    ...
    try {
        ...
        $bitmap.Save($env:EVE_CLIPBOARD_IMAGE, [System.Drawing.Imaging.ImageFormat]::Png)
...
function run() {
    ...
    const destination = $.NSProcessInfo.processInfo.environment.objectForKey('EVE_CLIPBOARD_IMAGE');
...
def read_clipboard_image():
    ...
    with tempfile.TemporaryDirectory(prefix="eve-clipboard-") as scratch:
        path = Path(scratch) / "image.png"
        env = {**os.environ, "EVE_CLIPBOARD_IMAGE": str(path)}
        try:

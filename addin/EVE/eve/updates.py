...
RELEASES = "https://github.com/10-X-eng/EVE/releases"
API = "https://api.github.com/repos/10-X-eng/EVE/releases?per_page=30"
...
def select_release(releases, current=VERSION, target=None):
    ...
    if not platform:
        raise ValueError("No EVE package is available for this platform.")
    ...
    for release in releases:
        ...
        filename = f"EVE-{number}-{platform}.zip"
...
def check_release():
    request = Request(API, headers={"Accept": "application/vnd.github+json",
                                   "User-Agent": f"EVE/{VERSION}", "X-GitHub-Api-Version": "2022-11-28"})
    ...
    def request(self):
        with self._lock:
            ...
            if self._thread is None:
                self._thread = threading.Thread(target=self._run, name="EVE-Updates", daemon=True)
                self._thread.start()
            else:
                self._wake.set()
        ...
        while True:
            ...
            try:
                ...
                result = {"updateChecking": False, "updateInfo": release,
                          "updateStatus": f"EVE {release['version']} is available" if release else "You’re up to date"}
            except Exception:

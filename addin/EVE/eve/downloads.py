...
def download_package(release, publish, cancelled, folder=None, opener=urlopen):
    ...
    filename = f"EVE-{version}-{platform}.zip"
    ...
    def request(address):
        return opener(Request(address, headers={"User-Agent": f"EVE/{VERSION}"}), timeout=15)
    ...
    def request(self, release):
        with self._lock:
            if self._closed.is_set() or (self._thread and self._thread.is_alive()):
                return
            self._thread = threading.Thread(target=self._run, args=(dict(release),), name="EVE-Download", daemon=True)
            self._thread.start()

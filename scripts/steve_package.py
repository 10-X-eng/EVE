...
from steve.version import VERSION  # noqa: E402
...
def package_name(target=None):
    ...
    if target not in PLATFORM_LABELS:
        raise RuntimeError(f"STEVE has no package definition for {target}. Supported: {', '.join(PLATFORM_LABELS)}")
    return f"STEVE-{VERSION}-{PLATFORM_LABELS[target]}"


def installer_name(target=None):
    return "Install STEVE.exe" if executable_suffix(target or host_target()) else "Install STEVE.command"
